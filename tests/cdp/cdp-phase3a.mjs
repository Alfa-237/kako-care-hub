...
async function main() {
  ...
  await goto("/familles");
  await waitFor(
    `document.body.innerText.includes("Gestion des familles")`,
    "familles apres migration",
  );
  await sleep(1000);
  ...
  let m = {};
  try {
    m = JSON.parse(migrated);
  } catch {}
  report(
    "T01",
    "Migration auto : entites Family derivees (24, par-001 actif)",
    m.count === 24 && m.par001?.name === "Famille Mbarga" && m.par001?.status === "Active",
    migrated,
  );
  ...
  report(
    "T02",
    "Liste familles rendue depuis les entites (24 lignes)",
    Number(rows) === 24,
    `rows=${rows}`,
  );
  ...
  report(
    "T03",
    "Bascule mosaique operationnelle (non-regression 2B)",
    t3 === "clicked" && Number(cards) === 24 && stored === "mosaique",
    `cards=${cards} stored=${stored}`,
  );
  ...
  await clickUntil(
    "button",
    "Vue liste",
    `[...document.querySelectorAll('button')].some(b => b.textContent.includes('Vue liste') && b.getAttribute('aria-pressed') !== 'true') || [...document.querySelectorAll('button[aria-label]')].some(b => (b.getAttribute('aria-label')||'').toLowerCase().includes('liste'))`,
    3,
  );
  await sleep(400);
  ...
  await clickUntil(
    '[role="dialog"] button',
    "Ajouter la famille",
    `document.body.innerText.includes("Famille creee") || location.pathname.startsWith("/familles/fam-")`,
    4,
  );
  await waitFor(
    `location.pathname.startsWith("/familles/fam-")`,
    "navigation vers fiche creee",
    15000,
  );
  await sleep(1500);
  ...
  let ac = {};
  try {
    ac = JSON.parse(afterCreate);
  } catch {}
  report(
    "T04",
    "Creation famille via dialog Zod -> navigation fiche + persistance",
    String(ac.path || "").startsWith("/familles/fam-") &&
      ac.rec?.status === "Active" &&
      ac.rec?.primary === null,
    afterCreate,
  );
  ...
  const tabSwitched = await clickUntil(
    '[role="tab"]',
    "Famille",
    `(() => { const t=[...document.querySelectorAll('[role=\"tab\"]')].find(x=>x.textContent.trim()==='Famille'); return t && t.getAttribute('aria-selected')==='true'; })()`,
  );
  await sleep(500);
  const ctaVisible = await ev(
    `[...document.querySelectorAll('button')].some(b => b.textContent.includes('Définir le responsable principal'))`,
  );
  ...
  report(
    "T05",
    "Nouvelle famille : CTA 'Definir le responsable principal' + 'Lier un enfant' desactive",
    tabSwitched === true && ctaVisible === true && linkDisabled === true,
    `tab=${tabSwitched} cta=${ctaVisible} disabled=${linkDisabled}`,
  );

  // ---- T06 : definition du responsable principal -> patch primaryParentId
  await clickUntil(
    "button",
    "Définir le responsable principal",
    `!!document.querySelector('#pf-first-name')`,
    4,
  );
  ...
  await clickUntil(
    ...
    `document.body.innerText.includes("Responsable ajouté") || !!document.querySelector('#pf-first-name') === false`,
    4,
  );
  await waitFor(
    `document.body.innerText.includes("Responsable ajouté")`,
    "toast responsable",
    15000,
  );
  await sleep(1500);
  ...
  let ps = {};
  try {
    ps = JSON.parse(primarySet);
  } catch {}
  report(
    "T06",
    "Responsable principal defini -> primaryParentId persiste",
    ps.match === true,
    primarySet,
  );
  ...
  await clickUntil(
    '[role="option"]',
    "Inactive",
    `[...document.querySelectorAll('[role="option"]')].some(o => o.getAttribute('aria-selected') === 'true' || o.getAttribute('data-state') === 'checked')`,
    4,
  );
  await sleep(300);
  await ev(
    `(() => { const n = document.querySelector('#ff-notes'); if (n) { n.focus(); n.select(); } })()`,
  );
  ...
  await clickUntil(
    '[role="dialog"] button',
    "Enregistrer",
    `document.body.innerText.includes("Famille modifiée")`,
    4,
  );
  ...
  await clickUntil(
    '[role="tab"]',
    "Infos",
    `(() => { const t=[...document.querySelectorAll('[role=\"tab\"]')].find(x=>x.textContent.trim().startsWith('Infos')); return t && t.getAttribute('aria-selected')==='true'; })()`,
  );
  await sleep(500);
  ...
  report(
    "T07",
    "Edition statut->Inactive : StatusPill + persistance apres rechargement",
  ...
  report(
    "T08",
    "'Lier un enfant' active une fois le responsable defini",
    linkEnabledNow === true,
    `enabled=${linkEnabledNow}`,
  );
  ...
  await sleep(500);
  await waitFor(
    `[...document.querySelectorAll('a')].some(a => a.textContent.includes('Mbarga'))`,
    "lien enfant Mbarga visible",
  );
  await clickUntil("a", "Mbarga", `location.pathname.startsWith('/enfants/')`, 4);
  await sleep(1200);
  const fromCtx = await ev(
    `location.pathname.startsWith('/enfants/') && location.search.includes('from=par-001') && [...document.querySelectorAll('button')].some(b => b.textContent.includes('Retour à la famille'))`,
  );
  ...
  await ev(
    `localStorage.setItem("kako.session.v1", JSON.stringify({ userId: "usr-003", startedAt: new Date().toISOString() })); "ok"`,
  );
  await goto("/familles");
  ...
  const eduNoEdit = await ev(
    `![...document.querySelectorAll('button')].some(b => b.textContent.trim() === 'Modifier')`,
  );
  report(
    "T10",
    "Educateur : ni 'Ajouter une famille' ni bouton Modifier",
    eduNoAdd === true && eduNoEdit === true,
    `add=${eduNoAdd} edit=${eduNoEdit}`,
  );

  // ---- T11 : reset.ts - resetData restaure le jeu complet
  await ev(
    `localStorage.setItem("kako.session.v1", JSON.stringify({ userId: "usr-001", startedAt: new Date().toISOString() })); "ok"`,
  );
  ...
  let rr = {};
  try {
    rr = JSON.parse(resetResult);
  } catch {}
  report(
    "T11",
    "resetData() reinitialise la base (familles incluses)",
    rr.families === 24 && rr.children === 24,
    resetResult,
  );
