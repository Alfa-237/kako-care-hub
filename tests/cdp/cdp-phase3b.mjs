...
async function main() {
  ...
  report(
    "T01",
    "Page présences rendue : stats + cartes enfants",
    Number(s1.cards) >= 15 && s1.attendus !== undefined,
    t1,
  );
  ...
  report(
    "T02",
    "Distribution du jour : present/absent/attendu/retard représentés",
    (d.present ?? 0) + (d["depart-anticipe"] ?? 0) > 0 &&
      (d.absent ?? 0) > 0 &&
      (d.attendu ?? 0) > 0 &&
      (d.retard ?? 0) > 0 &&
      total >= 15,
  ...
  report(
    "T03",
    "Recherche filtre puis réinitialisation restaure la liste",
    Number(searchCount) > 0 &&
      Number(searchCount) < Number(allBefore) &&
      Number(resetCount) === Number(allBefore),
  ...
  report(
    "T04",
    "Filtre statut Absent : uniquement des absents",
    ao.allAbsent === true && Number(ao.n) > 0,
    absentOnly,
  );
  ...
  let ta = null;
  try {
    ta = JSON.parse(targetAttendu);
  } catch {}
  let arrivalOk = false;
  if (ta?.childId) {
    await clickUntil(
      `button[aria-label="Pointer l'arrivée de ${ta.name}"]`,
      "",
      `!!document.querySelector('#arrival-time')`,
      6,
    );
    ...
    await clickUntil(
      ...
      `document.body.innerText.includes("Arrivée enregistrée") || !document.querySelector('#arrival-time')`,
      5,
    );
    await sleep(1500);
    const st = await ev(`(() => {
    ...
    arrivalOk =
      ["present", "retard"].includes(sv.cardStatus) &&
      ["present", "retard"].includes(sv.recStatus) &&
      !!sv.arrival;
  ...
  if (arrivalOk && ta) {
    await clickUntil(
      `[aria-label="Pointer le départ de ${ta.name}"]`,
      "",
      `!!document.querySelector('#departure-pickup')`,
      6,
    );
    await sleep(800);
    ...
    report(
      "T06",
      "Flux départ : récupérateur obligatoire + départ persisté",
      !!sd.pickup && !!sd.depTime,
      st,
    );
  } else {
  ...
  let tab2 = null;
  try {
    tab2 = JSON.parse(targetAbsent);
  } catch {}
  if (tab2?.childId) {
    await clickUntil(
      `[aria-label="Enregistrer l'absence de ${tab2.name}"]`,
      "",
      `!!document.querySelector('#absence-type')`,
      6,
    );
    ...
    await clickUntil(
      ...
      `document.body.innerText.includes("Absence enregistrée") || !document.querySelector('#absence-type')`,
      5,
    );
    await sleep(1500);
    const st = await ev(`(() => {
    ...
    report(
      "T07",
      `Flux absence (${tab2.name}) → absent + motif persisté`,
      sa.status === "absent" && sa.reason === "Test automatisé 3B" && !!sa.type,
      st,
    );
  } else {
  ...
  const pastLabelOk = await ev(
    `!document.body.innerText.toLowerCase().includes('lundi 24 août') || true`,
  );
  report(
    "T09",
    "Sélecteur de date : journée passée chargée avec historique",
    Number(pastRows) >= 10,
    `date=${pastDate} rows=${pastRows}`,
  );
  ...
  await clickUntil(
    '[role="tab"]',
    "Présences",
    `(() => { const t=[...document.querySelectorAll('[role=\"tab\"]')].find(x=>x.textContent.trim()==='Présences'); return t && t.getAttribute('aria-selected')==='true'; })()`,
  );
  await sleep(800);
  ...
  let sh = {};
  try {
    sh = JSON.parse(historyRows);
  } catch {}
  report(
    "T10",
    "Fiche enfant : onglet Présences affiche l'historique pointé",
    Number(sh.items) >= 3 && Number(sh.badges) >= 3,
    historyRows,
  );
  ...
  let sn = {};
  try {
    sn = JSON.parse(navOrder);
  } catch {}
  report(
    "T11",
    "Sidebar : Présences positionnée juste après Familles",
    sn.order === true,
    navOrder,
  );
  ...
  let se = {};
  try {
    se = JSON.parse(eduCanPoint);
  } catch {}
  report(
    "T12",
    "Éducateur : page fonctionnelle avec boutons de pointage",
    se.anyActionButton === true && se.placeholder === false,
    eduCanPoint,
  );
